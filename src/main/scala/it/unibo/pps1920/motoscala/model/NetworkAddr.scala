...
object NetworkAddr {
  ...
  def getLocalIPAddress: String = java.net.InetAddress.getLocalHost.getHostAddress
   ...
   */
  def validateIPV4Address(address: String): Boolean = address match {
   ...
   */
  private def validateIPV4Block(block: String): Boolean = block.nonEmpty && block
    .forall(_.isDigit) && (0 to 254 contains block.toInt)
